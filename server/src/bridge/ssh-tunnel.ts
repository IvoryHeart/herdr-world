...
export function classifySshTunnelFailure(
  ...
  if (
    ...
    return new SshTunnelError(
      "SSH host-key verification failed; verify the host outside Herdr World",
      false,
      "host-key",
      exitCode,
    );
...
export function assertSshTunnelPlatformSupported(
  ...
  if (platform === "win32") {
    throw new SshTunnelError(
      "SSH connections from Windows are not supported because Herdr World's stream-local forwarding cannot create a local Windows named pipe",
      false,
      "unsupported",
      -1,
    );
