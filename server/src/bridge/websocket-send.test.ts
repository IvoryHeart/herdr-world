...
test.skipIf(process.platform !== "darwin")(
  "WebKit accepts compressed terminal frames interleaved with plain RPC replies",
  async () => {
    const frame = JSON.stringify({
      terminal: {
        bytes: Buffer.from("\u001b[2Jterminal output\n".repeat(5000)).toString(
          "base64",
        ),
      },
    });
    const reply = JSON.stringify({ id: "rpc", result: { ok: true } });
    ...
    const server = Bun.serve({
      ...
      async fetch(request, server) {
        ...
        ws.onopen = () => ws.send('frames');
        ws.onclose = (event) => report({closed: event.code, received});
        ws.onmessage = (event) => {
          received.push(event.data === ${JSON.stringify(frame)} ? 'frame' :
            event.data === ${JSON.stringify(reply)} ? 'reply' : 'corrupt');
          if (received.length === ${expected.length}) {
            report({received, compressed: ws.extensions.includes('permessage-deflate')});
          }
        };
      </script>`,
          ...
          for (let i = 0; i < 20; i++) {
            for (const [payload, context] of [
              ...
              sendWebSocketMessage(ws, payload, {
                context,
                cleanup: () => result.reject(new Error("WebKit send failed")),
              });
            }
          }
        },
    ...
    try {
      view = new Bun.WebView({ backend: "webkit", dataStore: "ephemeral" });
      const received = await Promise.race([
        view.navigate(server.url.href).then(() => result.promise),
        new Promise<never>((_, reject) => {
          timer = setTimeout(
            () => reject(new Error("WebKit compression timed out")),
            15000,
          );
        }),
      ]);
      expect(received).toEqual({ received: expected, compressed: true });
    } finally {
      clearTimeout(timer);
      view?.close();
      server.stop(true);
