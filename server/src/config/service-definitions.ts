...
export const SERVICE_LABEL = "dev.herdr-world";
export const GENERATED_MARKER = "Generated by herdr-world service install.";
...
export const DEFAULT_SERVICE_ENV_FILE = `# herdr-world service environment
# This file is preserved by \`herdr-world service install\`.
HOST=0.0.0.0
PORT=8787
...
export function resolveServicePaths(
  ...
  if (platform === "windows-task") {
    ...
    const config = join(base, "herdr-world.env");
    ...
    return {
      config,
      definition: join(base, "herdr-world-task.ps1"),
      taskName: `${SERVICE_LABEL}-${userKey}`,
      stdoutLog: join(base, "herdr-world.stdout.log"),
      stderrLog: join(base, "herdr-world.stderr.log"),
    };
  }

  const config = join(homeDir, ".config", "herdr-world", "herdr-world.env");
  if (platform === "systemd") {
    return {
      config,
      definition: join(
  ...
  return {
    ...
    stdoutLog: join(homeDir, "Library", "Logs", "herdr-world.stdout.log"),
    stderrLog: join(homeDir, "Library", "Logs", "herdr-world.stderr.log"),
  };
}
...
// HERDR_WORLD default here: launchd already provides reliable supervisor detection.
