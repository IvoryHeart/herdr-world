...
import { assertSafeDataPath, publishDataFile } from "./data-paths";
...
function serviceHelp(): string {
  return `Manage Herdr World as a user service.

Usage:
  herdr-world service install [--force]
...
herdr-world.env file, and starts the service.
...
herdr-world. Uninstall preserves the environment file.
`;
}
...
function ensureStandaloneBinary(runtime: ServiceRuntime): string {
  ...
  if (
    ...
    throw new Error("service install requires the standalone herdr-world binary");
  }
...
function assertServiceDefinitionWritable(path: string, force: boolean) {
  if (existsSync(path)) {
    ...
    if (!existing.includes(GENERATED_MARKER) && !force) {
      throw new Error(
        `${path} was not generated by herdr-world; rerun with --force to replace it`,
      );
    }
  }
}
...
function assertNoLegacyService(
  ...
  if (serviceExists(platform, runtime, legacy, runCommand, "legacy")) {
    throw new Error(
      `legacy service detected (${legacy.definition}); stop and uninstall it using the previous binary's service uninstall before activating Herdr World. For custom definitions, explicitly disable and archive them yourself. Preserve the legacy config and token. See docs/DEPLOYMENT.md#transition-from-herdr-studio--herdr-gui. --force does not bypass this check.`,
    );
  }
}
...
function isPublishedLegacyDefinition(
  ...
  if (platform === "launchd") {
    expected = expected
      ...
      .replace(
        "<string>herdr-world-service</string>",
...
function managedServicePaths(
  ...
  if (
    ...
    if (serviceExists(platform, runtime, current, runCommand, "Herdr World")) {
      throw new Error(
        "both Herdr World and legacy services exist; resolve the duplicate services with the previous binary or native service manager before continuing; no service changes made",
      );
    }
    log(
      `Managing legacy service: ${legacy.definition}; its name and configuration are preserved. Uninstall it before installing the new Herdr World service.`,
    );
    return legacy;
  }
...
function installService(
  ...
  if (platform === "systemd") {
    ...
    if (code === 0) {
      code = runCommand(["systemctl", "--user", "enable", "herdr-world.service"]);
    }
    if (code === 0) {
      ...
      code = runCommand(["systemctl", "--user", "restart", "herdr-world.service"]);
    }
...
function uninstallService(
  ...
  if (
    ...
    throw new Error(
      `${paths.definition} was not generated by herdr-world and will not be removed`,
    );
  }
  if (platform === "systemd") {
...
export function runServiceCommand(
  ...
  try {
    ...
    if (command.action === "run") {
      ...
      if (worldEnv("RESTART_SUPERVISOR") === undefined) {
        process.env.HERDR_WORLD_RESTART_SUPERVISOR = "1";
      }
    ...
    error(`herdr-world service: ${(cause as Error).message}`);
    error("Run `herdr-world service --help` for usage.");
    return 1;
  }
}
