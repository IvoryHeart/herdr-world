...
function tempHome(): string {
  const path = mkdtempSync(join(tmpdir(), "herdr-world-service-"));
  tempDirs.push(path);
  return path;
}
...
function publishedLegacyDefinition(
  ...
  homeDir: string,
  binary = join(homeDir, "herdr-world"),
): string {
...
describe("service definition rendering", () => {
  test("escapes systemd paths without quoting them", () => {
    const definition = renderSystemdService(
      "/opt/Herdr % GUI/herdr-world",
      "/srv/herdr gui/config/herdr-world.env",
    );

    expect(definition).toContain(
      "EnvironmentFile=-/srv/herdr\\x20gui/config/herdr-world.env",
    );
    expect(definition).not.toContain('EnvironmentFile="');
    expect(definition).toContain(
      "ExecStart=/opt/Herdr\\x20%%\\x20GUI/herdr-world",
    );
  ...
  test("passes launchd paths as arguments without interpolating shell code", () => {
    const definition = renderLaunchdService("/Applications/Herdr & GUI", {
      config: "/srv/herdr & gui/config/herdr-world.env",
    ...
    expect(definition).toContain(
      "<string>/srv/herdr &amp; gui/config/herdr-world.env</string>",
    );
  ...
  test("renders a direct-executable Windows login task", () => {
    const definition = renderWindowsTaskDefinition(
      "C:\\Program Files\\Herdr's GUI\\herdr-world.exe",
      {
        config: "C:\\Users\\tester\\AppData\\Roaming\\herdr-world\\herdr-world.env",
        definition: "C:\\unused.ps1",
        taskName: "dev.herdr-world-test-user",
      },
    );

    expect(definition.charCodeAt(0)).toBe(0xfeff);
    expect(definition).toContain(
      "New-ScheduledTaskAction -Execute 'C:\\Program Files\\Herdr''s GUI\\herdr-world.exe'",
    );
    expect(definition).toContain(
      'service run "C:\\Users\\tester\\AppData\\Roaming\\herdr-world\\herdr-world.env"',
    );
    expect(definition).toContain("$taskName = 'dev.herdr-world-test-user'");
    expect(definition).toContain("-AllowStartIfOnBatteries");
    expect(definition).toContain("Generated by herdr-world service install.");
  });
  ...
  test("isolates Windows task names by user config directory", () => {
    ...
    expect(first.taskName).toStartWith("dev.herdr-world-");
...
describe("service commands", () => {
  test("installs and starts a systemd user service", () => {
    ...
    expect(commands).toEqual([
      ...
      {
        argv: ["systemctl", "--user", "enable", "herdr-world.service"],
        quiet: false,
      },
      {
        argv: ["systemctl", "--user", "restart", "herdr-world.service"],
        quiet: false,
      },
    ]);
    ...
    const configPath = join(homeDir, ".config", "herdr-world", "herdr-world.env");
    expect(readFileSync(definitionPath, "utf8")).toContain(
      "ExecStart=/opt/herdr-world-test/bin/herdr-world",
    );
    expect(readFileSync(configPath, "utf8")).toContain("HOST=0.0.0.0");
    expect(statSync(configPath).mode & 0o777).toBe(0o600);
    const tokenPath = join(homeDir, ".config", "herdr-world", "auth-token");
  ...
  test("reloads the systemd definition before restarting", () => {
    ...
    const code = runServiceCommand(["service", "reload"], {
      runtime: {
        ...
        homeDir,
        execPath: "/opt/herdr-world-test/bin/herdr-world",
        argv: ["/opt/herdr-world-test/bin/herdr-world", "service", "reload"],
      },
      runCommand: (argv) => {
        commands.push(argv);
        return 0;
    ...
    expect(commands).toEqual([
      ...
      ["systemctl", "--user", "restart", "herdr-world.service"],
    ]);
  });
  ...
  test("preserves an existing environment file during reinstall", () => {
    ...
    const code = runServiceCommand(["service", "install"], {
      runtime: {
        platform: "linux",
        homeDir,
        execPath: "/usr/local/bin/herdr-world",
        argv: ["/usr/local/bin/herdr-world", "service", "install"],
      },
      runCommand: () => 0,
      log: () => undefined,
    });
    ...
    expect(existsSync(join(homeDir, ".config", "herdr-world", "auth-token"))).toBe(
      false,
    );
  });
  ...
  test("preserves a managed custom wrapper command", () => {
    ...
    const binaryPath = join(homeDir, ".local", "bin", "herdr-world");
    ...
    writeFileSync(
      definitionPath,
      `# Generated by herdr-world service install.\n` +
  ...
  test("replaces a stale custom command that does not invoke this binary", () => {
    ...
    writeFileSync(
      ...
      `# Generated by herdr-world service install.\n` +
        ...
        `/opt/herdr-world-test/bin/herdr-world-old\n`,
    );
  ...
  test("validates launchd identity before writing service files", () => {
    ...
    const code = runServiceCommand(["service", "install"], {
      runtime: {
        ...
        homeDir,
        execPath: "/opt/herdr-world-test/bin/herdr-world",
        argv: ["/opt/herdr-world-test/bin/herdr-world"],
      },
      runCommand: () => {
    ...
    expect(() =>
      readFileSync(
        join(homeDir, ".config", "herdr-world", "herdr-world.env"),
        "utf8",
      ),
    ).toThrow();
    expect(() =>
      readFileSync(
        join(homeDir, "Library", "LaunchAgents", "dev.herdr-world.plist"),
        "utf8",
      ),
    ).toThrow();
  });
  ...
  test("installs, inspects, restarts, reloads, and removes a launchd agent", () => {
    ...
    const plistPath = join(
      ...
      "Library",
      "LaunchAgents",
      "dev.herdr-world.plist",
    );
    expect(() => readFileSync(plistPath, "utf8")).toThrow();
    expect(commands).toEqual([
      {
        argv: ["launchctl", "print", "gui/501/dev.herdr-world"],
        quiet: true,
      },
      {
      ...
      {
        argv: ["launchctl", "kickstart", "-k", "gui/501/dev.herdr-world"],
        quiet: false,
      },
      {
        argv: ["launchctl", "print", "gui/501/dev.herdr-world"],
      ...
      {
        argv: ["launchctl", "bootout", "gui/501/dev.herdr-world"],
        quiet: false,
      },
      {
        argv: ["launchctl", "bootstrap", "gui/501", plistPath],
  ...
  test("manages a Windows login task", () => {
    ...
    const executable = join(homeDir, "bin", "herdr-world.exe");
    ...
    expect(readFileSync(configPath, "utf8")).toContain("HOST=0.0.0.0");
    expect(
      readFileSync(join(appDataDir, "herdr-world", "auth-token"), "utf8").trim(),
    ).toMatch(/^[a-f0-9]{64}$/);
  ...
  test("uninstalls an inactive Windows task when its helper script is missing", () => {
    ...
    const code = runServiceCommand(["service", "uninstall"], {
      runtime: {
        ...
        homeDir,
        appDataDir,
        execPath: join(homeDir, "herdr-world.exe"),
        argv: [],
      },
      runCommand: (argv) => {
        commands.push(argv);
  ...
  test("propagates Windows task query failures without changing state", () => {
    const homeDir = tempHome();
    const appDataDir = join(homeDir, "AppData", "Roaming");
    const executable = join(homeDir, "herdr-world.exe");
    const paths = resolveServicePaths("windows-task", homeDir, appDataDir);
    const runtime = {
      platform: "win32",
      homeDir,
  ...
  test("loads environment for the direct Windows task process", () => {
    const homeDir = tempHome();
    const configPath = join(homeDir, "herdr-world.env");
    writeFileSync(
      configPath,
      [
        "# service environment",
    ...
    const keys = [
      ...
      "HERDR_WORLD_RESTART_SUPERVISOR",
    ] as const;
    ...
    try {
      ...
      expect(process.env.HERDR_WORLD_RESTART_SUPERVISOR).toBe("1");
    } finally {
  ...
  test("does not install a development runtime as a service", () => {
    ...
    expect(code).toBe(1);
    expect(errors.join("\n")).toContain(
      "service install requires the standalone herdr-world binary",
    );
  });
    ...
    for (const action of ["status", "restart", "reload", "uninstall"]) {
      test(`${platform} ${action} manages the generated legacy service without migrating it`, () => {
        ...
        expect(
          runServiceCommandWithLegacyCheck(["service", "install", "--force"], {
            runtime: {
              platform,
              homeDir,
              execPath: join(homeDir, "herdr-world"),
              argv: [],
              uid: 501,
            },
            runCommand: () => {
        ...
        const code = runServiceCommandWithLegacyCheck(["service", action], {
          ...
          runCommand: (argv) => {
            commands.push(argv);
            if (argv.includes("herdr-world.service")) return 4;
            if (argv.includes("gui/501/dev.herdr-world")) return 113;
            if (
...
describe("legacy management safety", () => {
    ...
    for (const state of [
      ...
      test(`${platform} refuses ambiguous or unverified management: ${state}`, () => {
        ...
        let definition = publishedLegacyDefinition(
          servicePlatform,
          homeDir,
          state === "another binary" ? "/other/herdr-world" : "/opt/herdr-world",
        );
  ...
  test("keeps the legacy launchd definition on query errors during uninstall or reload", () => {
    ...
    writeFileSync(
      legacy.definition,
      publishedLegacyDefinition("launchd", homeDir, "/opt/herdr-world"),
    );
    for (const action of ["uninstall", "reload"]) {
      ...
      expect(
        runServiceCommandWithLegacyCheck(["service", action], {
          ...
          runCommand: (argv) => {
            commands.push(argv);
            if (argv[2] === "gui/501/dev.herdr-world") return 113;
...
describe("legacy service activation preflight", () => {
  ...
  for (const platform of ["linux", "darwin", "win32"]) {
    test(`detects loaded legacy ${platform} service without its on-disk definition`, () => {
      ...
      expect(existsSync(join(homeDir, ".config", "herdr-world"))).toBeFalse();
    });
  }
  ...
  for (const args of [
    ...
    test(`confirmed launchd legacy absence permits ${args.join(" ")}`, () => {
      ...
      writeFileSync(
        current.definition,
        renderLaunchdService("/opt/herdr-world", current),
      );
      const commands: string[][] = [];
      expect(
        runServiceCommandWithLegacyCheck(args, {
  ...
  test("legacy Windows task hash stays tied to its old per-user config directory", () => {
    ...
    expect(current.taskName).toStartWith("dev.herdr-world-");
    expect(old.taskName?.split("-").at(-1)).not.toBe(
      current.taskName?.split("-").at(-1),
    );
  });

  test("after explicit legacy removal install starts fresh World env and token", () => {
    const homeDir = tempHome();
    const legacy = resolveLegacyServicePaths("systemd", homeDir);
    ...
    expect(readFileSync(current.config, "utf8")).toContain(
      "# herdr-world service environment",
    );
    expect(readFileSync(current.config, "utf8")).not.toContain(
      "HERDR_GUI_LOG_LEVEL",
    );
    expect(readFileSync(join(dirname(current.config), "auth-token"), "utf8"))
      .toMatch(/^[a-f0-9]{64}\n$/);
    expect(readFileSync(join(dirname(current.config), "auth-token"), "utf8"))
      .not.toBe(`${token}\n`);
    expect(
...
test("service run honors only the explicit World supervisor override", () => {
  const homeDir = tempHome();
  const configPath = join(homeDir, "herdr-world.env");
  const previousNew = process.env.HERDR_WORLD_RESTART_SUPERVISOR;
  try {
    delete process.env.HERDR_WORLD_RESTART_SUPERVISOR;
    writeFileSync(configPath, "HERDR_WORLD_RESTART_SUPERVISOR=0\n");
    expect(runServiceCommand(["service", "run", configPath])).toBe(
      SERVICE_COMMAND_CONTINUE,
    );
    expect(process.env.HERDR_WORLD_RESTART_SUPERVISOR as string | undefined).toBe(
      "0",
    );
    writeFileSync(configPath, "HERDR_WORLD_RESTART_SUPERVISOR=1\n");
