...
function scratch(): string {
  const dir = mkdtempSync(join(tmpdir(), "herdr-world-herdr-service-"));
  trash.push(dir);
  return dir;
}
afterEach(() => {
...
describe("resolveHerdrServicePaths", () => {
  test("uses per-platform user service locations", () => {
    ...
    expect(resolveHerdrServicePaths("systemd", homeDir).definition).toBe(
      join(homeDir, ".config", "systemd", "user", "herdr-world-herdr.service"),
    );
    ...
    expect(launchd.stdoutLog).toContain("herdr-world-herdr.stdout.log");
...
describe("service definition renderers", () => {
  ...
  test("windows task starts herdr server at logon from its install directory", () => {
    const script = renderHerdrWindowsTask("C:\\herdr\\herdr.exe", {
      ...
      taskName: "dev.herdr-world.herdr-abc123",
    });
    expect(script).toContain(HERDR_SERVICE_MARKER);
    expect(script).toContain("$taskName = 'dev.herdr-world.herdr-abc123'");
...
describe("installHerdrService", () => {
  test("writes the unit and runs the systemd activation sequence", () => {
    ...
    expect(commands).toEqual([
      ...
      ["systemctl", "--user", "enable", "herdr-world-herdr.service"],
      ["systemctl", "--user", "restart", "herdr-world-herdr.service"],
    ]);
  });
  ...
  test("refuses to replace a definition not generated by herdr-world", () => {
    const homeDir = scratch();
    ...
    ).toThrow("was not generated by herdr-world");
  });
});
