...
describe("last-step worktree snapshots", () => {
  ...
  test("does not delete capture storage after an uncertain failure", async () => {
    ...
    const runner: RunProcessWithCodeTimeout = async (argv, timeoutMs) => {
      ...
      if (failSnapshots && joined.includes("herdr-world-last-step-capture")) {
    ...
    try {
      ...
      expect(
        commands.some((command) =>
          command.includes("rm -f '/tmp/herdr-world-last-step-capture"),
        ),
      ).toBe(false);
      await baselines.dispose();
    } finally {
  ...
  test("queues a rapid second completion while the first is still finalizing", async () => {
    ...
    const runner: RunProcessWithCodeTimeout = async (argv, timeoutMs) => {
      if (argv.join(" ").includes("herdr-world-last-step-capture")) {
        snapshotCall += 1;
        if (snapshotCall === 2) {
  ...
  test("waits for step completion before publishing its replacement", async () => {
    ...
    const runner: RunProcessWithCodeTimeout = async (argv, timeoutMs) => {
      ...
      if (holdSnapshots && command.includes("herdr-world-last-step-capture")) {
        await snapshotGate;
      }
      return runProcessWithCodeTimeout(argv, timeoutMs);
    };
  ...
  test("retains the completed step when the next baseline capture fails", async () => {
    ...
    const runner: RunProcessWithCodeTimeout = async (argv, timeoutMs) => {
      if (
        failNextSnapshot &&
        argv.join(" ").includes("herdr-world-last-step-capture")
      ) {
