...
async function repository() {
  const root = await mkdtemp(join(tmpdir(), "herdr-world-snapshot-safety-"));
  await git(root, "init");
...
const quarantine = (root: string) =>
  join(root, ".git", "herdr-world-last-step-capture");
...
describe("bounded worktree snapshot transaction", () => {
  ...
  test("a raced ancestor symlink cannot redirect capture", async () => {
    ...
    const outside = await mkdtemp(
      join(tmpdir(), "herdr-world-snapshot-outside-"),
    );
    try {
