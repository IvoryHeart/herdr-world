...
const state = { schemaVersion: 3, id: randomUUID(), status: 'running', image: inspect.stdout.trim(),
  ...
  limits: { iterations: 12, failures: 3 }, remainingMs: 1800000, deadline: Date.now() + 1800000,
...
try {
  ...
  state.task = 'Review source.mjs: delay(0) must be 0, delay(1) 1000, delay(2) 2000, and all larger delays cap at 8000. Review only this helper; no external research or dependency setup is needed.';
  ...
  // First candidate has only the cap bug. Repair it, then introduce a new
  // immediate-retry bug so review cannot merely close the original finding.
  ...
  assert.equal(state.sessions.review.threadId, reviewId);
  assert.equal(state.turns.at(-1).model, 'gpt-5.6-luna');
  report.assertions.push('Repaired candidate passed review and Luna QA in the same independent history with a changed schema and model');
