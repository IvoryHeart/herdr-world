...
export async function prepareSmoke(output) {
  ...
  const spec = {
    ...
    tasks: [{ name: 'reconnect-smoke', metadata: { scope: 'tool-smoke', userBug: false },
      ...
      graders: [{ type: 'deterministic', run: 'python3 ' + quote(join(directory, 'grade-reconnect.py')), weight: 1 }] }],
  };
  ...
  await writeFile(join(output, 'inputs.json'), JSON.stringify({
    ...
    graderSha256: createHash('sha256').update(await readFile(join(directory, '../graders/grade.py'))).digest('hex'),
