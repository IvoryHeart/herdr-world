...
import { readFile, writeFile, mkdir, cp, access, open, unlink, rename, symlink, realpath } from 'node:fs/promises';
...
import { validateSessionGroups, fullSessionGroups, pairSessionGroups } from './sessions.mjs';
import { claimWorktree, recoverySource, carryRecovery } from './recovery.mjs';
import { inspectHealth, measureStorage } from './pair-health.mjs';
...
import { recordTask, readTask } from './task.mjs';
...
export function materializeConfig(config, control, seconds) {
  ...
  for (const hooks of Object.values(config.hooks.events)) for (const hook of hooks) hook.command[1] = join(control, hook.command[1]);
...
async function main() {
  const { values, positionals } = parseArgs({ allowPositionals: true, options: {
    ...
    'command-seconds': { type: 'string' }, 'prepare-only': { type: 'boolean', default: false },
  ...
  if (values.background) {
    if (!['start', 'resume', 'recover'].includes(action)) throw new Error('Only start/resume/recover can run in the background');
  ...
  let runDir, state;
  if (action === 'recover' && (values['task-file'] || values.interview || values['reference-image']?.length)) throw new Error('Recovery retains the accepted task. Add owner clarifications/images with resume after --prepare-only.');
  const recovery = action === 'recover' ? await recoverySource(runBase, positionals[1], process.cwd()) : null;
  if (['resume', 'status'].includes(action)) {
    ...
    if (action === 'status') {
      ...
      console.log(JSON.stringify({ id: state.id, status: state.status, reason: state.reason,
        activations: state.activations, remainingMs: state.workflow !== 'pair' && state.status === 'running' ? Math.max(0, state.deadline - Date.now()) : state.remainingMs,
        ...
        lastEvent: state.lastEvent, health: state.health, checks: state.checks, recovery: state.recovery, intake: state.intake, delivery: state.delivery }, null, 2));
      return;
    }
    if (!['interrupted', 'failed', 'blocked'].includes(state.status)) throw new Error('This outcome cannot resume; use recover for explicitly authorized additional work');
    if (values.model || values.image || values.iterations || values.seconds || values.profile || values['auth-file']
      ...
      || values['command-seconds'] || values['prepare-only']
      || values['reasoning-effort'] || values['task-profile'] || values.sessions || values.interview) {
      throw new Error('Resume preserves frozen controls, model, environment and limits; use recover for an explicitly authorized change');
    }
    ...
    if (![3, 4, 5].includes(state.schemaVersion)) throw new Error('This run uses the previous workflow; use recover to load updated controls while preserving work');
    if (state.workflow === 'pair') {
      const task = await readTask(state.delivery.worktree);
      if (task?.runId !== state.id || task.branch !== git(['branch', '--show-current'], state.delivery.worktree) || task.branch === 'main'
        || await realpath(join(runDir, 'workspace')) !== await realpath(state.delivery.worktree)) throw new Error('Resume must match the recorded task branch, worktree and current run');
      releaseWorktree = await claimWorktree(state.delivery.worktree, state.id);
    }
    if (values['task-file']) {
      ...
      state.lastEvent = state.workflow === 'pair' ? 'plan.start' : initialEvent(state.taskProfile);
      if (state.workflow === 'pair') state.pair = {};
      if (state.intake) state.intake.ready = false;
    } else if (state.workflow === 'pair') {
      if (state.pair?.approval?.fingerprint === await fingerprint(join(runDir, 'workspace'))) state.lastEvent = state.verification?.status === 'passed' ? 'lead.start' : 'pair.agreed';
      else state.lastEvent = state.resumeEvent ?? (state.requirements ? 'pair.a' : 'plan.start');
    ...
    if (state.workflow !== 'pair' || values['task-file']) invalidateCandidate(state);
  } else if (action === 'start' || recovery) {
    if (!values['task-file'] && !recovery) throw new Error('Usage: agent:run -- start --task-file <file> [--task-profile routine|feature|sensitive] [--model <all-role override>] [--profile check|acceptance] [--iterations 24] [--seconds 3600]');
    const branch = git(['branch', '--show-current']);
    if (!branch || branch === 'main') throw new Error('Start from a non-main task worktree');
    if (git(['status', '--porcelain']) && recovery?.state.workflow !== 'pair') throw new Error('Commit the input first; runs require a clean source snapshot');
    const profile = values.profile ?? recovery?.state.profile ?? 'check';
    if (!['check', 'acceptance'].includes(profile)) throw new Error('Run profile must be check or acceptance');
    const taskProfile = values['task-profile'] ?? recovery?.state.taskProfile ?? 'routine';
    ...
    const workflow = values.workflow ?? 'pair';
    if (workflow === 'pair' && await realpath(process.cwd()) === await realpath(primaryCheckout())) throw new Error('Pair execution needs a linked task worktree; use agent:goal or agent:worktree create first.');
    if (recovery && workflow !== 'pair') throw new Error('Recovery upgrades to the pair workflow');
    if (workflow === 'pair' && !recovery && (await readTask())?.runId) throw new Error('This task already has a run. Resume it, or recover it to update controls; do not reset the task allowance.');
    ...
    if (recovery && !['model', 'worker-model', 'lead-model', 'oracle-model', 'reasoning-effort'].some(key => values[key])) {
      const previous = recovery.state.models;
      const mapping = recovery.state.workflow === 'pair' ? Object.fromEntries(Object.keys(models).map(role => [role, role]))
        : { intake: 'intake', planner: 'planner', lead: 'planner', 'pair-a': 'implementer', 'pair-b': 'reviewer', oracle: 'oracle' };
      for (const [role, oldRole] of Object.entries(mapping)) if (previous?.[oldRole]) models[role] = previous[oldRole];
    }
    ...
    const image = values.image ?? recovery?.state.image ?? 'herdr-world-agent:1';
    ...
    const authFile = resolve(values['auth-file'] ?? recovery?.state.authFile ?? join(homedir(), '.codex/auth.json'));
    await access(authFile);
    if (recovery && values.seconds === undefined && recovery.state.remainingMs < 1000) throw new Error('Previous model allowance is exhausted; recovery needs explicitly authorized --seconds');
    if (recovery && values.iterations === undefined && recovery.state.limits.iterations <= recovery.state.activations) throw new Error('Previous activations are exhausted; recovery needs explicitly authorized --iterations');
    const limits = { iterations: positive(values.iterations, recovery ? recovery.state.limits.iterations - recovery.state.activations : 24, 64),
      seconds: positive(values.seconds, recovery ? Math.floor(recovery.state.remainingMs / 1000) : 3600, 86400), failures: 3 };
    if (workflow === 'pair') releaseWorktree = await claimWorktree(process.cwd(), id);
    state = { schemaVersion: workflow === 'pair' ? 5 : 4, id, status: 'preparing', sourceRevision: git(['rev-parse', 'HEAD']),
      sourceFingerprint: await fingerprint(), model: values.model ?? null, models, taskProfile, lastEvent: workflow === 'pair' ? 'plan.start' : initialEvent(taskProfile), image: inspect.output.trim(), profile, authFile,
      sessionMode, workflow, commandTimeoutMs: positive(values['command-seconds'], 2700, 7200) * 1000, budgets: budgetPolicy(limits.seconds),
      ...
      sessionGroups: validateSessionGroups(workflow === 'pair' ? pairSessionGroups : workflow === 'full' ? fullSessionGroups : policy.sessionGroups), sessions: {},
      task: recovery?.state.task ?? await readFile(values['task-file'], 'utf8'), intake: values.interview ? { ready: false } : null,
      delivery: recovery?.state.delivery ?? { worktree: process.cwd(), base: process.env.WORLD_AGENT_BASE ?? 'main', parent: process.env.WORLD_AGENT_PARENT ?? null }, limits, activations: 0, consecutiveFailures: 0,
    ...
    await cp(join(repoRoot, 'harness'), join(control, 'harness'), { recursive: true, verbatimSymlinks: true,
      filter: path => workflow !== 'pair' || !path.startsWith(join(repoRoot, 'harness/node_modules')) });
    ...
    if (workflow === 'pair') {
      state.workspace = await realpath(process.cwd());
      state.gitCommonDir = git(['rev-parse', '--path-format=absolute', '--git-common-dir']);
      await symlink(state.workspace, join(runDir, 'workspace'));
      try { state.reviewBase = git(['merge-base', state.delivery.base, 'HEAD']); } catch { state.reviewBase = state.sourceRevision; }
    } else await copyCandidate(process.cwd(), join(runDir, 'workspace'));
    state.workspaceBaseline = git(['rev-parse', 'HEAD'], join(runDir, 'workspace'));
    if (recovery) await carryRecovery(recovery, runDir, state);
    ...
    if (workflow === 'pair') await measureStorage(runDir, state.workspace, 'start');
    if (values['prepare-only']) {
      state.status = 'interrupted'; state.reason = 'Recovery/start prepared; no model or checks started';
      await saveState(runDir, state); console.log('Prepared run: ' + id); return;
    }
    if (workflow === 'pair') await inspectHealth(runDir);
    const prepared = await prepareDependencies(state, join(runDir, 'workspace'), control, { timeoutMs: state.workflow === 'pair' ? state.commandTimeoutMs : 1200000 });
    if (prepared.code !== 0) {
    ...
    state.status = 'running';
    await saveState(runDir, state);
  } else throw new Error('Actions: start, resume <id>, recover <id>, status <id>');
  ...
  try {
    ...
    if (state.workflow === 'pair') {
      if (action !== 'resume') {
        try { await rename(join(runDir, 'workspace/.ralph'), join(runDir, 'previous-ralph')); } catch (error) { if (error.code !== 'ENOENT') throw error; }
      } else {
        const prepared = await prepareDependencies(state, join(runDir, 'workspace'), control, { timeoutMs: state.commandTimeoutMs });
        await saveState(runDir, state);
        if (prepared.code !== 0) { state.status = 'blocked'; state.reason = 'Preparation failed; resume after repairing the environment'; await saveState(runDir, state); throw new Error(state.reason); }
      }
    }
    ...
    // Pair model time excludes deterministic preparation/checks. Each command keeps
    // its own safety deadline; this outer ceiling only catches supervisor failures.
    const runtimeMs = state.workflow === 'pair' ? state.remainingMs + state.commandTimeoutMs * 10 : state.remainingMs;
    const config = materializeConfig(parse(await readFile(join(control, 'harness', state.workflow === 'pair' ? 'ralph-pair.yml' : 'ralph.yml'), 'utf8')), control, Math.ceil(runtimeMs / 1000));
    // Resume starts a new event ledger; our persisted activation budget remains authoritative.
    config.event_loop.max_iterations = state.limits.iterations - state.activations + (state.workflow === 'pair' ? 2 : 0); // Ralph checks its ceiling before consuming completion: allow emission and dispatch, both model-free.
    ...
    if (action === 'resume') {
      ...
      try { await cp(join(previous, 'agent'), join(runDir, 'workspace/.ralph/agent'), { recursive: true }); } catch (error) { if (error.code !== 'ENOENT') throw error; }
    }
    config.event_loop.max_runtime_seconds = Math.max(1, Math.floor(runtimeMs / 1000));
    ...
    { cwd: join(runDir, 'workspace'), env: { ...process.env, WORLD_AGENT_RUN: runDir },
      timeoutMs: runtimeMs, graceMs: state.budgets ? 10000 : 0, log: join(runDir, 'ralph.log') });
    state = await loadState(runDir);
    ...
    if (result.interrupted) { state.status = 'interrupted'; state.reason = 'Supervisor interrupted'; }
      ...
      (result.timedOut || (state.workflow === 'pair' && state.remainingMs <= 0) || state.activations >= state.limits.iterations || state.consecutiveFailures >= state.limits.failures)) {
    ...
    if (state.workflow === 'pair') await measureStorage(runDir, state.workspace, 'stop');
    ...
    for (const attempt of usage.attempts.filter(a => a.role !== 'verifier')) {
      ...
      if (turn) { turn.usage = attempt.usage; turn.usageCoverage = attempt.coverage; }
        ...
        if (state.workflow === 'pair') state.remainingMs = Math.max(0, state.remainingMs - attempt.elapsedMs);
        state.turns.push({ role: attempt.role, model: attempt.model, reasoningEffort: attempt.reasoningEffort,
...
if (process.argv[1] === fileURLToPath(import.meta.url)) main().catch(errorExit).finally(() => releaseWorktree?.());
