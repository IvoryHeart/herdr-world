...
import type { WorldGraphNode } from "./graph/herdrGraphProjection";
...
export type WorldThemeContext = WorldSurfaceContext & {
  ...
  onGraphOpenTerminal: (node: WorldGraphNode) => void;
  onGraphOpenInSpaces: (node: WorldGraphNode) => void;
};
...
export function isWorldThemeContext(value: unknown): value is WorldThemeContext {
  ...
  return Boolean(record.graphProjection) &&
    typeof record.onGraphSelect === "function" &&
    typeof record.onGraphOpenTerminal === "function" &&
    typeof record.onGraphOpenInSpaces === "function";
}
