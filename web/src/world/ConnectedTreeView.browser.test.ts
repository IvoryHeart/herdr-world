...
test.skipIf(!chrome).each([1280, 390])(
  "the connected Tree preserves its hierarchy and interactions at %ipx",
  async (width) => {
    const dir = await mkdtemp(join(tmpdir(), "connected-tree-test-"));
    ...
    const server = Bun.serve({
      ...
      async fetch(request) {
        ...
        if (path === "/") {
          return new Response(
            '<head><link rel="stylesheet" href="/ConnectedTreeView.browser.css"></head><body><script type="module" src="/ConnectedTreeView.browser.js"></script></body>',
    ...
    try {
      const build = await Bun.build({
        entrypoints: [join(import.meta.dir, "ConnectedTreeView.browser.tsx")],
        outdir: dir,
        target: "browser",
        plugins: [
          {
      ...
      const failures = await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          timer = setTimeout(
            () => reject(new Error("Connected Tree browser check timed out")),
            30_000,
          );
        }),
      ]);
