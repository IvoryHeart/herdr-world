...
export class GhosttyRenderer implements TerminalRenderer {
  ...
  async mount(container: HTMLElement) {
    ...
    terminal.attachCustomKeyEventHandler((event) => handleTerminalCustomKeyEvent(event, terminal));
  ...
  #installMobileInputBridge() {
    ...
    const onCompositionEnd = (event: CompositionEvent) => {
      ...
      if (endedState.phase === "idle" && endedState.pendingInput?.kind === "cancellation") {
        return;
      }
      queueMicrotask(() => {
