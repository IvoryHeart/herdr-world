...
describe("SidebarToolbar", () => {
  it("exposes the four View destinations and keeps Space scope separate", async () => {
  ...
  it("dispatches Tree and names its compact reopen control", async () => {
    const onOpenCurrentView = vi.fn();
    const rendered = await renderToolbar({
      activeWorldTheme: { id: "tree" },
      onOpenCurrentView,
    });
    const view = rendered.container.querySelector<HTMLSelectElement>("[aria-label='View']")!;
    expect(view.value).toBe("tree");
    const reopen = rendered.container.querySelector<HTMLButtonElement>(
    ...
    expect(rendered.onView).toHaveBeenCalledExactlyOnceWith("office");
    await act(async () => reopen?.click());
    expect(onOpenCurrentView).toHaveBeenCalledOnce();
  });
