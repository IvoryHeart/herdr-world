...
- [ ] 1.4 Make the one Spaces Inspector follow the active tab and pane, including tab-strip/window/sibling focus, and fence prior tab/pane resource replies. When its Terminal resource is selected, present a Focus tab window action instead of a terminal attachment; the Spaces tab window owns its Herdr split or zoom presentation in Single and multiwindow modes, and the action focuses the selected pane.
