...
First verify Herdr 0.9.0 can report bounded pane tokens, expose the active agent-session identity and deliver updates to World. Use the same service/CLI transport proven for task summaries. Add a narrow `herdr-world agent-checkout` report command for an explicit pane, absolute checkout path and optional HTTPS PR URL. The producer may run once as a harness hook on the target host; it must not inspect browser focus. Herdr 0.9.0's [metadata contract](https://github.com/herdrdev/herdr/blob/v0.9.0/docs/next/website/src/content/docs/socket-api.mdx) limits one token to 80 characters and one report to 16 keys; its `agent`/`applies_to_source` guards do not protect token patches. Do not claim that those guards bind checkout tokens to a session.

### Let the exact agent session own report lifetime

At report time, read the target pane's active `agent_session` (`source`, `agent`, `kind`, `value`) and store the lowercase hex SHA-256 of `JSON.stringify([source, agent, kind, value])` alongside the checkout fields. Reject a missing or malformed field; do not normalize the identity before hashing. World recomputes that fingerprint from the current Herdr pane/session before displaying or querying Git. Omit `ttl_ms`: Herdr retains token metadata until replacement, pane closure or server restart. A long-running unchanged session needs no timer or renewal hook; a replacement session immediately fails the fingerprint check even if the old tokens remain on the pane. A new report replaces all chunks atomically and clears unused old chunks in that same metadata call. A failed or unsupported session read fails the report; it never falls back to workspace or terminal CWD.

Do not expose `--clear` or run an end-of-session cleanup in the first release. These 15 keys are pane-global. A delayed clear from session A after session B's one-shot report would erase B's context, and Herdr does not provide a conditional token delete by the expected session fingerprint. The per-source `seq` facility requires a shared monotonic sequence that independent one-shot hooks do not have. World hides A's retained tokens immediately when A ceases to be the active session; B's report replaces them, and pane closure or Herdr restart removes them. Within an unchanged session, a bad checkout can be corrected by a new full report but cannot be retracted immediately in this release. A former hook invoking `--clear` receives a usage error before any Herdr request, including after B has reported. If the product later needs immediate physical erasure of tokens on session end, first design a serialized, session-conditional owner rather than adding a blind cleanup command.
...
- [An old session's cleanup erases a newer report] → Offer no Clear or post-session cleanup; reject `--clear` before metadata access and let session matching prevent stale display.
