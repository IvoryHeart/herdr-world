...
- [ ] 2.1 Add a dedicated task-summary argument parser for report, `--pane`, `--session` and `--ttl-ms`; reject `--clear` with a usage error before any Herdr request. Verify focused cases for defaults, missing pane, invalid TTL and a summary beginning with `-` after `--`.
