...
- [x] 8.2 Replace the checkpoint selection panel with one shell-owned selected-entity Inspector overlay that leaves the Office stage dimensions unchanged, presents compact identity and one close lifecycle without a duplicate actionable profile card, and uses a read-only activation context only for inactive or stale selections; atomically retire and admit identity/resources and verify delayed, rejected and generation-replaced targets cannot expose mismatched or operational fallback content
- [x] 8.3 Reuse the existing Files, Changes and Agent History components and resource state as the overlay body without cloning an Inspector or runtime store, immediately expose only the selected entity's applicable tabs, default terminal-capable selections to Terminal, retain the chosen tab locally, remove the redundant Inspector-local Open in Spaces shortcut, and verify spaces and non-agent panes retain their promised Inspector access while cross-host selection neither switches the connection nor retains prior live resources
...
- [x] 9.1 Present one focused terminal through the current Roamgate-derived `TerminalView` in either the Inspector's Terminal tab or a floating conversation target, suppress a competing mounted-but-hidden Spaces view, expose unambiguous Open terminal window and Dock in Inspector icon controls, make Inspector close close rather than open a window, and verify selection away from a docked agent detaches the old target before mounting its replacement while preserving one qualified attachment without duplicate input or transport
...
- [x] 9.3 Restore direct Office desk activation and up to five independent selected-host desktop conversation windows alongside the one Inspector, with bounded move, resize, order, close/focus, stable portal publication, real terminal refit and validated browser-local geometry; verify a desk opens or focuses its floating terminal, terminals support side-by-side copy/paste, and duplicate and sixth-window behavior remain bounded
- [x] 9.4 Restore separate qualified connectors from floating terminals to their desk/agent/node anchors and from the Inspector overlay to its agent anchor, plus explicit handoff to the mounted Spaces pane; verify connectors track scene, overlay and window movement without changing topology, selection or terminal identity
...
- [x] 11.3 Integrate the shared selected-host Inspector context and qualified terminal conversations while retaining read-only detail for other hosts and an equivalent bounded compact/assistive hierarchy; verify inactive/stale branches expose no operational action even when another host uses the same display label, plus task summaries and keyboard/touch operation
