...
- [ ] 8.1 Complete the qualified WorldObject presenter input with bounded admitted task summaries, agent/model and state labels, focus, normalized tab labels, active/ready-inactive/reconnecting/offline-stale host state and action capabilities; retain selected identity with its observed generation and verify missing metadata is never inferred or rebound across generations
- [ ] 8.2 Replace the checkpoint selection panel with one shell-owned selected-agent intent overlay that leaves the Office stage dimensions unchanged, opens on selection with compact icon/name/safety state, immediately exposes History, Files, Changes and Terminal, and draws a qualified connector to the agent; verify inactive, stale and generation-replaced targets preserve captured read-only identity without operational fallback
- [ ] 8.3 Reuse the existing Files, Changes and Agent History components and resource state as the overlay body without cloning an Inspector or runtime store, default new agent selection to History, retain the chosen tab locally, and verify selecting an entity on another host neither switches the connection nor retains live resources from the prior selected-host context
