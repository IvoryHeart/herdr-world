...
- [x] 8.2 Replace the checkpoint selection panel with one shell-owned selected-entity intent overlay that leaves the Office stage dimensions unchanged, presents compact agent identity and an agent connector for agents, and presents compact capability-derived context for spaces and non-agent panes; verify inactive, stale and generation-replaced targets preserve captured read-only identity without operational fallback
- [x] 8.3 Reuse the existing Files, Changes and Agent History components and resource state as the overlay body without cloning an Inspector or runtime store, immediately expose only the selected entity's applicable tabs, default new agent selection to History, retain the chosen tab locally, and verify spaces and non-agent panes retain their promised Inspector access while cross-host selection neither switches the connection nor retains prior live resources
...
- [ ] 9.1 Present one focused Office terminal through the current Roamgate-derived `TerminalView` in either the intent overlay's Terminal tab or a floating conversation target, suppress a competing mounted-but-hidden Spaces view, and verify Dock in profile, Pop out and selection away from a docked agent detach the old target before mounting its replacement while preserving one qualified attachment without duplicate input or transport
- [ ] 9.2 Extend that terminal owner with a bounded registry keyed by selected connection, runtime generation and terminal identity inside the inherited browser routing lease, and verify the registry adds no Herdr Web terminal implementation, connection-independent client, second WebSocket, SSH tunnel, application store or per-host terminal manager
- [ ] 9.3 Restore direct Office desk activation and up to five independent selected-host desktop conversation windows with bounded move, resize, order, close/focus, real terminal refit and validated browser-local geometry, and verify a desk opens or focuses its floating terminal plus duplicate and sixth-window behavior
