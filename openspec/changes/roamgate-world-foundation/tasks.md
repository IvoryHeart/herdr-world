...
- [ ] 8.2 Replace the checkpoint selection panel with one shell-owned selected-entity intent overlay that leaves the Office stage dimensions unchanged, presents compact agent identity and an agent connector for agents, and presents compact capability-derived context for spaces and non-agent panes; verify inactive, stale and generation-replaced targets preserve captured read-only identity without operational fallback
