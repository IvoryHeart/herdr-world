...
Snapshot results include a per-host watch-admission summary tied to the watchlist revision: registered, missing, unresolved, uniquely matched live, admitted and admission-failed record counts. Classify each registered record exactly once from the host's raw `pane.list`: zero exact terminal-ID candidates is missing; multiple candidates, a malformed candidate or invalid workspace/tab ancestry is unresolved; exactly one candidate with valid ancestry is matched. An admitted record is a matched record retained with its complete ancestry; any matched record not admitted is an admission failure and must be shown, not called missing or unresolved. For a fresh current-generation host, enforce `registered = missing + unresolved + matched` and `matched = admitted + admission_failed`. For example, one watched terminal ID duplicated in two raw panes yields registered 1, unresolved 1 and zero for every other category. A stale or unfinished host has no current missing, unresolved, matched or admitted classification. Pin/Unpin invalidates aggregate observation; an in-flight result built under an older watchlist revision cannot satisfy a newer list, so queue a generation-fenced follow-up refresh. Browsers show admission as pending until the snapshot and watchlist revisions match.
